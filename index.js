...
function addGamesToPage(games) {
    ...
    // Loop over each item in the gamesArray
    games.forEach(game => {
...
const unfundedGames = GAMES_JSON.filter(game => game.pledged < game.goal);
const numberOfUnfundedGames = unfundedGames.reduce((count, game) => count + 1, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = 
`A total of $${totalPledged.toLocaleString()} has been raised for ${totalGames} game${totalGames !== 1 ? 's' : ''}. 
Currently, ${numberOfUnfundedGames} game${numberOfUnfundedGames !== 1 ? 's' : ''} remain${numberOfUnfundedGames !== 1 ? '' : 's'} unfunded. 
